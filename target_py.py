...
cpn['opnum'].nunique()
cpn[cpn['free']]['opnum'].nunique()

round(cpn[cpn['free']]['opnum'].nunique() / cpn['opnum'].nunique(), 3)

# Grants

cpn['free'].sum()
(cpn['free'].sum() / len(cpn)).round(3)
sum(cpn['mean_ncs5'] == 0)
...
pt = (
  cpn.groupby('free')[['mean_ncs5', 'topic_switch', 'log_prev_mean_ncs5', 'log_seniority']]
  .describe(percentiles=percs)
  .stack(level=0)          
  .reset_index()           
...
pt['Grant type'] = np.where(pt['free'], 'Open', 'Targeted')
...
cpn['free_label'] = np.where(cpn['free'], 'Non-targeted', 'Targeted')
